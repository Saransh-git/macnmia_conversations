...
order_level_one_month = pd.read_parquet("/Users/saransh/Desktop/practicum/data/lagone.parquet")
...
order_level_one_month['customer_tokenized_msg'] = order_level_one_month.customer_tokenized_msg.parallel_apply(
    str_to_tokens)
order_level_one_month['stylist_tokenized_msg'] = order_level_one_month.stylist_tokenized_msg.parallel_apply(
    str_to_tokens)

random.seed(60616)
msg_data_ind = list(order_level_one_month.index)
random.shuffle(msg_data_ind)  # shuffle so as to allow a better optimum/ fit via stochastic gradient in neural network

train_doc_list = order_level_one_month.loc[msg_data_ind]
...
skip_gram_model.delete_temporary_training_data(False, False)  # free up memory
dm_model_concat.delete_temporary_training_data(False, False)  # free up memory
order_level_one_month.drop('index', axis=1, inplace=True)
order_level_one_month.to_parquet('/Users/saransh/Desktop/practicum/data/lagonetrain.parquet', index=False)
